...
struct StackElement {
  ...
  char    mateDetected;      // if == 1 mate is detected and valable till the end of the game
  ...
  int     PawnHash;   // Current Pawn Hash initiated at each search and updated on PlayMove
...
struct StackElement2 {
  ...
  int    ponders[100]; // list of ponders corecponding to Attacks list, indexed by rankOnLevel
