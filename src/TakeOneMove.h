int LiteEvaluation(int move,int ahead,int depth);

int AplyExcengeLogic(int SqFrom,int SqTo,int PieceMoved,int PieceTaken,int Indicators,int ahead,int depth);
int StockAttack(int from, int to, int moved, int taken, int indi);
...
int TakeOneMove(int alpha,int beta,int ahead,int depth,int sortedlevel)
{
...
wstack2  = (struct StackElement2*)&Stack2[gameDepth];
  ...
  if(wstack2->Attacks[0] == 99) return 0;
  ...
  if(sortedlevel == 0 )
  {
	...
	if(gameDepth > treeDepth )
	{
        ...
        for(n=treeDepth;n<gameDepth;n++)
		{
			...
			if(BestLine[n - treeDepth] != Stack2[n].move) break;
	...
	if(gameDepth == treeDepth && best == 0 && BestLine[0] > 0 && BestLine[0] > 65535)
	{
			best = BestLine[0];
	}
	
//	if(best == 0) best = Stack2[gameDepth].bestmove; e mai bun BestLine
  ...
  if(wstack2->moveBestLine > 0)
  {
	...
	// if found a move copy it to MoveToDo and suprim it with the last move from list 
	if(ix >= 0)
		  {
...
// merge noAttacks with Attacks, ponder moves and sort them
  ...
  // now main white and black hits are set	  
	...
	{		    
			...
			if(Indicators > 0 )
				{
					if(PieceTaken > 0 ) ran = 121; // transformare cu luare de piesa
					else ran = 120; // transformare simpla
				}			
			if(PieceTaken > 0)
			{
				int v1 = abs(ValPiesa[ PieceTaken]);
				int v2 = abs(ValPiesa[ PieceMoved]);				
				if(PieceMoved == WK || PieceMoved == BK) ran = 122;
				else if(v1  - 400 > v2) ran = 124;
				else if(v1  - 200 > v2) ran = 123;
				else if(v1   > v2) ran = 122;

				else if(PieceMoved == WQ && PieceTaken == BQ) ran = 119; // good moves
				else if(PieceMoved == BQ && PieceTaken == WQ) ran = 119;
			
				else if(PieceMoved == WR && PieceTaken == BR) ran = 118;
				else if(PieceMoved == BR && PieceTaken == WR) ran = 118;	
			
				else if(PieceMoved == WN && PieceTaken == BB) ran = 117;
				else if(PieceMoved == BN && PieceTaken == WB) ran = 117;
			
				else if(PieceMoved == WB && PieceTaken == BB) ran = 116;
				else if(PieceMoved == BB && PieceTaken == WB) ran = 116;
			
				else if(PieceMoved == WB && PieceTaken == BN) ran = 115;
				else if(PieceMoved == BB && PieceTaken == WN) ran = 115;
			
				else if(PieceMoved == WP && PieceTaken == BP) ran = 114;
				else if(PieceMoved == BP && PieceTaken == WP) ran = 114;

			}
		if (ran == 100)
		{
 		
			if(mo == Stack2[gameDepth].killers[0]  ) ran = 110;
			if(mo == Stack2[gameDepth - 2].killers[0] && gameDepth > 2 ) ran = 108;
			
			else if( mo == Stack2[gameDepth].killers[1] ) ran = 106;
			else if(mo == Stack2[gameDepth - 2].killers[1]  && gameDepth > 2) ran = 104;
			
			else if(PieceMoved  < 7 && History.whiteMateHistory[mm] != 0) ran = 109; // mate seen from this move
			
			else if(PieceMoved >= 7 &&  History.blackMateHistory[mm] != 0) ran = 109;
			
			else if(PieceMoved  == WP && SqTo > 70) ran = 107; // pawn push
			else if(PieceMoved  == BP && SqTo < 40) ran = 107;
			else if(PieceMoved  == WP && Stack[gameDepth].pieceCounts[WP] <= 4) ran = 104; // pawn to move
			else if(PieceMoved  == BP && Stack[gameDepth].pieceCounts[BP] <= 4) ran = 104;
	...
	{
		case WK:
			if(tabla_black_power_attacks[SqFrom] > 0) { xran =  -10000 ; }
			if(tabla_black_power_attacks[SqTo] > 0) { ran = - 800000 - 10000 ;  break;}
			else 	ran = xran + 100 + PieceTaken; 		
			break;
		case WQ:
		case WR:
		case WB:
		case WN:
			if(tabla_black_power_attacks[SqFrom] > 0) { xran +=   800 * PieceMoved ; }
			if(tabla_black_power_attacks[SqTo] > 0 && 
				abs(ValPiesa[PieceMoved]) > abs(ValPiesa[PieceTaken])) 
			...
			break;		
		case BK:
			if(tabla_white_power_attacks[SqFrom] > 0) { xran =  -10000 ; }
			if(tabla_white_power_attacks[SqTo] > 0 ) { ran = - 800000 - 10000 ;  break;}
			else 
		...
		case BB:
		case BN:
			if(tabla_white_power_attacks[SqFrom] > 0) { xran +=  800 * PieceMoved ; }
...
if (ran == 100)
{
	...
	wstack2->ponders[ikk] = ran ;			
		 ...
		 for (d = start; d < nn; d++ )
		 {
			...
			if(iVal > max) {max = iVal; mo = tMove;j = d;}
		 }
  ...
  if (noTime == 1) return 0; // no moves on levels
  return 0;
}
...
// return 0; // nu, caci nu mai pot taia pe latime
	...
	int eval = Stack[gameDepth].ponder + Stack[gameDepth].positionVal; // material
if (ahead == 1)
{
		int penalty = 0;
		int xx = 0;
	...
	if(treeDepth < 30 && (int)PieceMoved == WK ) eval -= 100;
	if(treeDepth < 30 && (int)PieceMoved == BK ) eval -= 100;

	// penalties for queens early moves
	if(treeDepth < 20 &&  PieceMoved == WQ) eval -= 500;
	if(treeDepth < 20 &&  PieceMoved == BQ) eval -= 500;

	// stimul for pawn moves in midle game
	if(treeDepth > 50 &&  PieceMoved == WP) eval += 100;
	if(treeDepth > 50 &&  PieceMoved == BP) eval += 100;
    	
	if(PieceTaken > 0) return eval;

	int nPieces = Stack[gameDepth].nPieces;
	
	// Penalties for queen at A4
	if(PieceMoved == WQ  && SqTo == 51) eval -= 800;
...
else
{
	...
	case WN :
		eval += (DistKing[SqFrom][(int)Stack[gameDepth].blackKingSquare] -
			DistKing[SqTo][(int)Stack[gameDepth].blackKingSquare]) * 10 ;
		
		break;
	case BN :
		eval += (DistKing[SqFrom][(int)Stack[gameDepth].whiteKingSquare] -
			DistKing[SqTo][(int)Stack[gameDepth].whiteKingSquare]) * 10 ;
		break;
	...
	case BB :
		eval += (DistKing[SqFrom][(int)Stack[gameDepth].whiteKingSquare] -
			DistKing[SqFrom][(int)Stack[gameDepth].whiteKingSquare]) * 10 ;
	...
	else if(r1 > 0  &&  (pi1 + pi2) == 1) bonus = 10; // bonus for rooks protecting/ attacking pawns
	eval += bonus;
	...
	bonus = 0;
	if(r2 > 0  &&  (pi1 + pi2) == 1) bonus += 10; // bonus for rooks protecting/ attacking pawns
	eval += bonus;
	...
	bonus = 0;
	if(q1 > 0  &&  (pi1 + pi2) == 1) bonus = 10; // bonus for queens protecting/ attacking pawns
	...
	bonus = 0;
	if(q2 > 0  &&  (pi1 + pi2) == 1) bonus -= 10; // bonus for queens protecting/ attacking pawns
...
if(depth > 0)
{
  int evalxx = AplyExcengeLogic(SqFrom,SqTo,PieceMoved,PieceTaken,Indicators,ahead,depth);
 eval += evalxx;
}
...
if(Stack[gameDepth].pieceCounts[WP] > 0)
{
	...
	for(it=0;it< Stack[gameDepth].pieceCounts[WP];it++)
	{
		...
		if(tabla[loc + 9] >= 0) tabla_white_power_attacks[loc + 9] += pondere_piesa[WP];
		if(tabla[loc + 11] >= 0) tabla_white_power_attacks[loc + 11] += pondere_piesa[WP];		
	}
...
if(Stack[gameDepth].pieceCounts[BP] > 0)
{
	for(it=0;it< Stack[gameDepth].pieceCounts[BP];it++)
	{
		...
		if(tabla[loc - 9]  >= 0) tabla_black_power_attacks[loc - 9] += pondere_piesa[BP];
		if(tabla[loc - 11] >= 0) tabla_black_power_attacks[loc - 11] += pondere_piesa[BP];		
	}
}
...
if(tabla[k1 - 11] >= 0) tabla_white_power_attacks[k1 - 11] += pondere_piesa[WK];
if(tabla[k1 - 10] >= 0) tabla_white_power_attacks[k1 - 10] += pondere_piesa[WK];
if(tabla[k1 - 9] >= 0) tabla_white_power_attacks[k1 - 9] += pondere_piesa[WK];
if(tabla[k1 - 1] >= 0) tabla_white_power_attacks[k1 - 1] += pondere_piesa[WK];
...
if(tabla[k1 - 11] >= 0) tabla_black_power_attacks[k1 - 11] += pondere_piesa[WK];
if(tabla[k1 - 10] >= 0) tabla_black_power_attacks[k1 - 10] += pondere_piesa[WK];
if(tabla[k1 - 9] >= 0) tabla_black_power_attacks[k1 - 9] += pondere_piesa[WK];
if(tabla[k1 - 1] >= 0) tabla_black_power_attacks[k1 - 1] += pondere_piesa[WK];
...
if(Stack[gameDepth].pieceCounts[WN] > 0)
{
	for(it = 0; it < Stack[gameDepth].pieceCounts[WN];it++)
	{
		...
		if(tabla[k1 - 21] >= 0) tabla_white_power_attacks[k1 - 21] += pondere_piesa[WN];
		if(tabla[k1 - 19] >= 0) tabla_white_power_attacks[k1 - 19] += pondere_piesa[WN];
		if(tabla[k1 - 12] >= 0) tabla_white_power_attacks[k1 - 12] += pondere_piesa[WN];
		if(tabla[k1 - 8] >= 0) tabla_white_power_attacks[k1 - 8] += pondere_piesa[WN];
...
if(Stack[gameDepth].pieceCounts[BN] > 0)
{
	for(it = 0; it < Stack[gameDepth].pieceCounts[BN];it++)
	{
		...
		if(tabla[k1 - 21] >= 0) tabla_black_power_attacks[k1 - 21] += pondere_piesa[WN];
		if(tabla[k1 - 19] >= 0) tabla_black_power_attacks[k1 - 19] += pondere_piesa[WN];
		if(tabla[k1 - 12] >= 0) tabla_black_power_attacks[k1 - 12] += pondere_piesa[WN];
		if(tabla[k1 - 8] >= 0) tabla_black_power_attacks[k1 - 8] += pondere_piesa[WN];
			...
			for(;;)
			 {
			   ...
			   if ( (int)tabla[loc2] < 0) break;// e margine

			   tabla_white_power_attacks[loc2] += pondere_piesa[WQ];
   
			   if ( (int)tabla[loc2] > 0) break;// e piesa
			} // end a direction
			...
			for(;;)
			 {
			   ...
			   if ( (int)tabla[loc2] > 0) break;// e o piesa
			} // end a direction
		} // end for directions for a piece
