...
int contor_variante;
int
DepuneVarianta (int ponderea, int indicatori);
int
RegasireVarianta (int caz);
int apel_depuneri;
int gasit_depuneri;
...
DepuneVarianta (int ponderea, int indicatori)
{
  ...
  if (departarea != 3)
    return 0;
  if (Stack2[treeDepth].IsCheckOwn != 0
      || Stack2[treeDepth + 1].IsCheckOwn != 0
      || Stack2[treeDepth + 2].IsCheckOwn != 0
      || Stack2[treeDepth + 3].IsCheckOwn != 0)
    return 0;
  if (Stack2[treeDepth].IsCheck != 0 || Stack2[treeDepth + 1].IsCheck != 0
      || Stack2[treeDepth + 2].IsCheck != 0
      || Stack2[treeDepth + 3].IsCheck != 0)
    return 0;
  ...
  for (id = 0; id < contor_variante; id++)
    {
      ...
      if (m3 != variante[id][3])
        continue;
      // gasit varianta; daca indicatori == 1 return cai protejam variante
      // analizate complet
      if (indicatori != 1)
        return 0;
...
RegasireVarianta (int caz)
{
  ...
  indi_return
      = variante[id][5]; //  indicatori daca e plin sau taiere de arbore
  return 0;
  return 1;
}
