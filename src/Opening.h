...
struct OpeningText OpeningText[] = 
{
"1  e2-e4", 
"1a e7-e5",
"2  g1-f3",
...
if((tabla[25] != 6) || (tabla[95] != 12)) return 1;
...
// 1. Process primary text file
    ...
    for(i=0;;i++)
	{
		...
		// isolate a word
		if(cmove[0] == 'x') break; // no more opening moves
        k = 0;
		TakeWord(nb, cmove,0);
		...
		if ((nb[1] != 'a') && (nb[1] > 0))
		{
			if((nb[1] < '0') || (nb[1] > '9')) err++;

			if((nb[1] >= '0') && (nb[1] <= '9'))
				n =  n * 10  + ( nb[1] - 48) * 2;
		    if(nb[2] == 'a') n++; // black  move
		}
		if(nb[1] == 'a') n = n + 1;
		...
		ch = tmove[2];
        if(( ch != '-' ) && (ch != 'x')) err++; // convert to lowercase
...
// 2. Map the moves to actual move from game
	...
	for(nbOpen=0;nbOpen < nbOpeningMoves;nbOpen++)
	{
		if(OpeningBinary[nbOpen].moveNumber < gameDepth)
		{
			...
			{
				MoveBack();
				if(OpeningBinary[nbOpen].moveNumber == gameDepth) break;
			}
		};
		...
		if(SqFrom == 25 && SqTo == 27) MoveToDo += (1 << 24);
		if(SqFrom == 95 && SqTo == 97) MoveToDo += (2 << 24);
...
if(SqFrom == 25 && SqTo == 26)
{
	who = who;
		...
		GenLevelMoves(0);
		if(SqFrom == 25 && SqTo == 23) MoveToDo += (1 << 24);
		if(SqFrom == 95 && SqTo == 93) MoveToDo += (2 << 24);

		aa = AllowedMove(MoveToDo, MoveToDo);

		if(aa == 0)
		{
			MessageBox(hWnd, " Exista o mutare imposibila !!!!!!!", " EROARE FISIER DESCHIDERE ", MB_OK);
		}
		else
		{
...
int MoveToDo;
 ...
 int OpeningCandidates[100]; // here store all moves possible to continuing opening
...
// return 0;
 ...
 if(gameDepth > 50) return 0; // no longer openings than 25 full plyes
 ...
 for(i=0;i<120;i++)
	 ta[i] = tabla0[i];
// 1. Select all possible moves from opening file posible from current chessboard position
	...
	for(nbOpen=0;nbOpen < nbOpeningMoves;nbOpen++)
	{
		if(gDepth > OpeningBinary[nbOpen].moveNumber )
		{
			...
			MoveBackTa(); 
			who = ( who + 1) & 1;
			if(gDepth == OpeningBinary[nbOpen].moveNumber) break;
			}
		};
...
//	  CopyMove(&OpeningCandidates[randmove], &MoveToDo);
	...
	if(aa == 0)
	{
		MessageBox(hWnd, " Deschidere o mutare imposibila !!!!!!!", " EROARE FISIER DESCHIDERE ", MB_OK);
	}
	else
	{
...
int TakeWord(char  text[], char cmove[], int mode)
{
  int j;
  j = 0;
  char ch;
