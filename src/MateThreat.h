//
// Slow Chess move generation functions
// by Jon Kreuzer
//
//
// MOVELIST FUNCTIONS
...
// -----------------
void inline AddCastleMove (const int src, const int dst, const int type, const int piece)
        {
        int indi = 1;
		if piece == BK indi = 2;

		Stack[gameDepth].NoAttacks[Stack[gameDepth].nbNoAttacks++].move = piece * 1000000 + src * 100 + dst;
...
// -----------------
void FindMovesWhitePromotedPawn ( Movestruct *WMoves, SBoard &Board, unsigned char *Boardptr, int data)
        {
         if     (*Boardptr == WQUEEN)   FindMovesWhiteQueen (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == WROOK)   FindMovesWhiteRook  (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == WBISHOP) FindMovesWhiteBishop(*WMoves, Board.Sq, Boardptr - Board.Sq, data);
		 else if (*Boardptr == WKNIGHT) FindMovesWhiteKnight(*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == WPAWN)   FindMovesWhitePawn  (*WMoves, Board.Sq, Boardptr - Board.Sq, data, Board.EnPassant);
         }

void FindMovesBlackPromotedPawn ( Movestruct *WMoves, SBoard &Board, unsigned char *Boardptr, int data)
        {
         if      (*Boardptr == BQUEEN)  FindMovesBlackQueen (*WMoves, Board.Sq, Boardptr- Board.Sq, data);
         else if (*Boardptr == BROOK)   FindMovesBlackRook  (*WMoves, Board.Sq, Boardptr- Board.Sq, data);
         else if (*Boardptr == BBISHOP) FindMovesBlackBishop(*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == BKNIGHT) FindMovesBlackKnight(*WMoves, Board.Sq, Boardptr- Board.Sq, data);
         else if (*Boardptr == BPAWN)   FindMovesBlackPawn  (*WMoves, Board.Sq, Boardptr- Board.Sq, data, Board.EnPassant);
         }    

void FindAMovesWhitePromotedPawn ( Movestruct *WMoves, SBoard &Board, unsigned char *Boardptr, int data)
        {
         if      (*Boardptr == WQUEEN)  FindAMovesWhiteQueen    (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == WROOK)   FindAMovesWhiteRook     (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == WBISHOP) FindAMovesWhiteBishop   (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == WKNIGHT) FindMovesWhiteKnight    (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == WPAWN)   FindMovesWhitePawn      (*WMoves, Board.Sq, Boardptr - Board.Sq, data, Board.EnPassant);
         }

void FindAMovesBlackPromotedPawn ( Movestruct *WMoves, SBoard &Board, unsigned char *Boardptr, int data)
        {
         if      (*Boardptr == BQUEEN)  FindAMovesBlackQueen  (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == BROOK)   FindAMovesBlackRook   (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == BBISHOP) FindAMovesBlackBishop (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == BKNIGHT) FindAMovesBlackKnight (*WMoves, Board.Sq, Boardptr - Board.Sq, data);
         else if (*Boardptr == BPAWN)   FindAMovesBlackPawn   (*WMoves, Board.Sq, Boardptr - Board.Sq, data, Board.EnPassant);
         }         
*/
// -----------------------------------
// Find all the moves on a Board for WHITE and store in Attacks or NoAttacks at level gameDepth
// -----------------------------------
void FindMovesWhite(int indExchange)
        {
		int i,j,np;
        Clear_Movelist(); 
...
void FindHeavyMovesWhite ( int color)
{
		Clear_Movelist(); 
...
// -------------------------------------------------
void FindMovesBlack(int indExchange)
        {
         Clear_Movelist();
...
*/
// Rook and Queen Moves only
void FindHeavyMovesBlack (int color)
{
	    int i,j,np;
        Clear_Movelist();
...
//		indExchange = 22;
		...
		for(i=1;i<=6;i++)
		{
			...
			for(j=0;j<np;j++)
			{
				GenMovesForAWhitePiece(22,i, Stack[gameDepth].pieceAPosition[i][j]);
			}
		}
		Close_Movelist();
//		indExchange = 0;
	 ...
	 for(i=7;i<=12;i++)
		{
			...
			for(j=0;j<np;j++)
			{
				GenMovesForABlackPiece(22,i, Stack[gameDepth].pieceAPosition[i][j]);
			}
		}
	Close_Movelist();
//	indExchange = 0;
...
int inline IsAttacked (int square, int color)
   ...
   if (color == WHITE) 
		{
		if (DistKing[square][Stack[gameDepth].blackKingSquare] <= 1) return 1;
		...
		if (DistKing[square][Stack[gameDepth].whiteKingSquare] <= 1) return 1;
...
//	 else	{AddMove (x+10, Move );
 ...
 if ((tabla [ x+11 ] < 8 && tabla  [ x+11 ] > 0) || (x+11) == nSqEnPassant)  AddAtkMove (x+11, Move );
 if ((tabla  [ x+9 ] < 8  && tabla [ x+9 ] > 0)  || (x+9)  == nSqEnPassant)  AddAtkMove (x+9,  Move );
}
...
{
	for (int i = 0; i < 8; i++)
		if ( tabla[ x + dn[i] ] < BP && tabla[ x + dn[i]  ] > 0) AddAtkMove (x + dn[i], Move );
}
...
{
	for (int i = 0; i < 8; i++)
		if ( tabla[ x + dk[i] ] < BP && tabla[ x + dk[i]  ] > 0) AddAtkMove (x + dk[i], Move );
}
...
{
 if ( (tabla [ x+11 ] < BP && tabla [ x+11 ] > 0) || (x+11) == nSqEnPassant)  AddAtkMove (x+11, Move );
 if ( (tabla [ x+9 ]  < BP && tabla [ x+9  ] > 0) || (x+9)  == nSqEnPassant)  AddAtkMove (x+9, Move );
...
void inline AddMoveW (int src,int dst,int bAttack, int indi)
	...
	nPiece = tabla[src];
	nPiece2 = tabla[dst];
	if ( bAttack == 2 || nPiece2 == BK || !DiscoveredCheck (WHITE, dst | ( src << 8) | ( nPiece2 << 16) | (nPiece << 20) | (indi << 24) )) 
		{

				*pma = dst | ( src << 8) | ( nPiece2 << 16) | (nPiece << 20) | (indi << 24);
				*pma++;

		}
	}

void inline TestAddDirecW (int sq, int nDirec,int piece1, int piece2, int bAttack )
{
	int tempSq = sq + nDirec;
	while (tabla [tempSq] == 0)	tempSq+= nDirec;
	if (tabla [ tempSq ] == piece1 || tabla [ tempSq ] == piece2) {AddMoveW (tempSq, sq,bAttack, 0 );}
}
//
// Find the moves
//
void inline FindWhiteMovesToSq (int sq,int bAtk)
{
	...
	if (bAtk > 0)
	{
		...
		if (Stack2[gameDepth].enPassant == (sq) && tabla[sq + 10] == 0 && tabla[(int)Stack2[gameDepth].enPassant] == BP)
			{
			indi = 3;
			if (tabla [ sq- 1 ] == WP) 
				{
...
void inline AddMoveB (int src, int dst, int bAttack, int indi)
	...
	if ( bAttack == 2 ||  nPiece2 == WK || !DiscoveredCheck (BLACK,dst | ( src << 8) | ( nPiece2 << 16) | (nPiece << 20) | (indi << 24) )) 
		{
...
void inline TestAddDirecB (int sq, int nDirec, int piece1,int piece2,int bAttack )
{
	int tempSq = sq + nDirec;
	while (tabla [tempSq] == 0)	tempSq+= nDirec;
	if (tabla [ tempSq ] == piece1 || tabla [ tempSq ] == piece2) {AddMoveB (tempSq, sq, bAttack, 0 );}
}
...
//				AddMoveB ( sq+9,  sq, 1, 12 ); 
	...
	if (Stack2[gameDepth].enPassant == (sq) && tabla[sq-10] == 0 && tabla[(int)Stack2[gameDepth].enPassant] == WP)
		{
		if (tabla [ sq- 1 ] == BP) 
			{
...
int *pma10;
 ...
 if ( IsCheck(color) > 0) 
 {if (who == WHITE) FindBlackMovesToSq ( Stack[gameDepth].whiteKingSquare, 2);
		else FindWhiteMovesToSq (Stack[gameDepth].blackKingSquare, 2);
 nmoves = (pma - pma10) ;
	 if (nmoves > 1) ret = 2;
	 else if( nmoves == 1)
	 {
...
// ------------------------
int CheckSquareW (int MateSq, int nKingSq, int nQueenSq )
{
int indRet;
int piece;
return 0;
MateSq = MateSq + nKingSq;
if (tabla[ MateSq ] > 0 && tabla[MateSq]< BP) return 0; // Friendly piece

int nDirec = DirKing [ MateSq][nQueenSq];
if (nDirec == 0) return 0;
if (CheckLine (MateSq, nQueenSq, nDirec, BQ, BQ) == 0) return 0;
...
if ( TablaBlackIsCheck ( tabla[ MateSq ] )) indRet = 0; // Other Piece can take queen
tabla[nQueenSq] = piece;

if (indRet == 0) return 0;
//if (DiscoveredCheck (WHITE, nKingSq, nQueenSq, MateSq, 0 ,0) ) return 0; // Queen is Pinned
...
//
//
int CheckSquareB (int MateSq, int nKingSq, int nQueenSq )
{
int piece;
return 0;
MateSq = MateSq + nKingSq;
if ( tabla[MateSq]> WK) return 0; // Friendly piece blocking

int nDirec = DirKing[ MateSq][nQueenSq];
// Is the queen lined up with the mating square? If so does it have a clear path?
if (nDirec == 0) return 0;
if (CheckLine (MateSq, nQueenSq, nDirec, WQ, WQ) == 0) return 0;
...
if ( TablaWhiteIsCheck ( tabla[ MateSq ] )  ) indRet = 0; // Other Piece can take queen
tabla[nQueenSq] = piece;

if (indRet == 0) return 0;

// if (DiscoveredCheck (BLACK,nKingSq, nQueenSq , MateSq, 0, 0) ) return 0; // Queen is Pinned
...
int Qcode;
int nKingSq;
if (depth > 4) return FindMateThreat ( (who + 1) & 1);

nKingSq = Stack[gameDepth].whiteKingSquare;
if(who == BLACK) 
{   
	if(Stack[gameDepth].pieceCounts[BQ] == 0) return 0;
...
{
	if(Stack[gameDepth].pieceCounts[WQ] == 0) return 0;
...
if (MateSq == 0) return 0;

if (CheckSquareW ( MateSq, Ksq, Qsq ) == 1) return 1;

if (MateSq == 9)
	{
	 if (CheckSquareW (  -1, Ksq, Qsq ) == 1) return 1;
	 if (CheckSquareW ( +10, Ksq, Qsq ) == 1) return 1;
	 return 0;
	}
if (MateSq == 11)
	{
...
if (CheckSquareB ( MateSq, Ksq, Qsq ) == 1) return 1;

if (MateSq == -9) 
	{if (CheckSquareB (   1, Ksq, Qsq ) == 1) return 1;
	 if (CheckSquareB ( -10, Ksq, Qsq ) == 1) return 1;
	 return 0;
	}
if (MateSq == -11)
	{
...
if (MateSq== -10) 
	...
	if (tabla[Ksq-1] > 0 && tabla[Ksq-1] <BP && (tabla[Ksq-10] == 0 || 
...
int anothermoveonlevel;
 ...
 
	{
	 ...
	 if (DiscoveredCheck (Color, MoveToDo ) == 1 ) continue;
...
int IsCheck(int color)
	...
	if (DistKing[Stack[gameDepth].whiteKingSquare ][Stack[gameDepth].blackKingSquare ] <= 1 )
		 {
		 if (color == WHITE) 
...
int inline CheckDirec ( int Sq0, int nDirec, int piece1, int piece2)
{
	int Sq;
	Sq = Sq0 + nDirec;
	while (tabla[Sq] == 0) Sq +=nDirec;
	if (tabla[Sq] == piece1 || tabla[Sq] == piece2) return Sq;
	return 0;
}
...
int DiscoveredCheck ( int color, int Move)
	{
	 int bRetVal = FALSE; 
...
// TestWrong();
			cPiece = tabla [ srcSq ]; 
            enPassant = 0;
			if ((dstSq + 10) == Stack2[gameDepth].enPassant && tabla[srcSq] == BP && tabla[dstSq] == 0 && tabla[(int)Stack2[gameDepth].enPassant] == WP)
				{
			...
			tabla [ srcSq ] = cPiece;
			if(enPassant != 0) tabla[enPassant] = WP;
// TestWrong();
			}
		}
	 else
...
// TestWrong();
			cPiece = tabla [ srcSq ];
			enPassant = 0;
			if ((dstSq - 10) == Stack2[gameDepth].enPassant && tabla[srcSq] == WP && tabla[dstSq] == 0 && tabla[(int)Stack2[gameDepth].enPassant] == BP)
				{
				// en passant vacate 2 squares
				enPassant = Stack2[gameDepth].enPassant; 
				tabla[ enPassant ] = 0;
			...
			tabla [ srcSq ] = cPiece;
			if(enPassant != 0) tabla[enPassant] = BP;
// TestWrong();
...
int inline DiscoverCheckB (int Sq, int KingSq, int nDirec)
{
	...
	if (nDirecD != 0 && nDirecD != nDirec) 
	{
		if ( nDirecD == -1 || nDirecD == 1 || nDirecD == -10 || nDirecD == 10)	
			...
			else 	return CheckDirec (KingSq , nDirecD,BB, BQ); 
	}
	return 0;
}
...
// returns the checking Square, (or returns the number 2 if there are 2 checking squares)
//
// ----------------------------------------------------------

int CheckingMove( int color, int Move )
{
	int pie;
...
if(tabla[srcSq] == 0)
{
	...
	if (tabla[srcSq] == BK )
		...
		if (srcSq == 95 && dstSq == 97)  {dstSq1 = 96; srcSq1 = 98;}
		if (srcSq == 95 && dstSq == 93)  {dstSq1 = 94; srcSq1 = 91;}
	...
	if (tabla[srcSq] == WK )
		{
		// white rocades
		if (srcSq == 25 && dstSq == 27)  {dstSq1 = 26; srcSq1 = 28;}
		if (srcSq == 25 && dstSq == 23)  {dstSq1 = 24; srcSq1 = 21;}
...
if (color == WHITE)
	...
	if (piece == WP && SQRANK[dstSq]==8) { tabla[dstSq] = Promo[indi];  }
// tabla[srcSq] is empty now!!
	switch ( tabla[dstSq] )
		{
		case WP:
			if ((dstSq == KingSq-9) || (dstSq == KingSq-11)) nCheck = dstSq;
			//En Passant captures vacate 2 squares, so we have to have to check the captured piece square for discovered too
			else if ( (dstSq - 10) == Stack2[gameDepth].enPassant && oldpiece == 0) 
				{
				tabla [dstSq-10] = 0;
				
				nCheck=DiscoverCheckW (dstSq-10, KingSq, DirKing [ KingSq][dstSq - 10]);
		...
		case WB:
			...
			break;
		case WN:
			if (KnightKing [ dstSq][ KingSq ] & 1 ) nCheck = dstSq;
			break;
		case WR:
			if ( nDirec == -1 || nDirec == 1 || nDirec == -10 || nDirec == 10) 
			...
			break;
		case WQ:
			if (nDirec) nCheck = CheckLine ( KingSq, dstSq, nDirec, WQ, WQ);
			break;
		}

	// Discovered Checks
	if ( piece != WQ) nDCheck = DiscoverCheckW (srcSq, KingSq, nDirec);
	}
// Same as above, except for BLACK
	...
	int nDirec = DirKing [ KingSq][dstSq];
	int indi = (MoveToDo >> 24) & 15;;
	if (piece == BP && SQRANK[dstSq]==1) { tabla[dstSq] = Promo[indi];  }
// tabla[srcSq] is empty now!!
	...
	switch ( tabla[dstSq] )
		...
		case BP:
			...
			{
				tabla [dstSq+10] = 0;
				nCheck = DiscoverCheckB (dstSq+10, KingSq, DirKing [ KingSq][dstSq + 10]);
				tabla [dstSq+10] = WP;
				}
			break;
		case BB:
		...
		case BR:
			...
			break;
		case BQ:
			if (nDirec) nCheck = CheckLine ( KingSq, dstSq, nDirec, BQ, BQ);
			break;
		}

	// Discovered Checks
	if ( piece != BQ) nDCheck = DiscoverCheckB (srcSq, KingSq, nDirec);
	}

tabla [dstSq] = oldpiece;
tabla [srcSq] = piece;

if (nCheck==0) return nDCheck;
else if (nDCheck) return 2; // 2 Checking Pieces
return nCheck;
}
...
if ((DirKing[Ksq][Qsq]) != 0 )
	...
	dir = DirKing[Ksq][Qsq];
	TempSq = Ksq + dir;
	while (tabla[TempSq] == 0) TempSq+=dir;
