...
BOOL CALLBACK
PawnDlgProc (HWND hwnd, unsigned msg, UINT wparam, LONG lparam);
...
int nModels = 4;
char modelList[4][20]
    = { "model4.bmp", "model1.bmp", "model2.bmp", "model3.bmp" };
...
int SqSize = SQSIZE;
int IconBits[3 * ICONSIZE * ICONSIZE
             * 200]; // 3 colors  of SqSize x SqSize icons for 200 icons
...
int
ReadRepositoryEnd (int);

int
image_read (char bu[], int nIcon, int IconSet);

int
MyClearWind (void);
int
MyBarRect (int x1a, int y1a, int x2b, int y2b, COLORREF co1);
int
MyBarRect1 (int x1a, int y1a, int x2b, int y2b, COLORREF co1);
int
MyTextOut (int x1a, int y1a, char* buf1, COLORREF co1);

int
MyLine (int x1a, int y1a, int x2b, int y2b, COLORREF co2);
int
MyCircle (int xcentru, int ycentru, int raza, COLORREF col1);

int
InitChessBoard ();

int
DrawChessPiece (int IconSet, int Square, int pie);
int
RedrawChessBoard ();
...
int
MoveToString (int Move, char moveout[]);
...
MyRegisterClass (HINSTANCE hInstance)
{
  ...
  (LPCTSTR)IDI_BUSINESSCHESS); wcex.hCursor		= LoadCursor(NULL,
  IDC_ARROW); wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
...
*/
  ...
  if (!xxx)
    {
      MessageBox (NULL, "Class registration has failed!", "Error!",
                  MB_OK | MB_ICONERROR);
      return 0;
    }
  return 1;
...
InitInstance (HINSTANCE hInstance, int nCmdShow)
{
  ...
  hWnd = CreateWindowEx (0, "ME",
                         "Friendly Chess - Program de Sah Prietenos Pentru "
                         "Fiecare Vesr. 2023 3.1",
                         WS_OVERLAPPEDWINDOW, 0, 0, /* left corner position */
                         cx, cy, /* width & height */
...
WndProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (message)
    ...
    case WM_SHOWWINDOW:
      ...
      sta = 1;
      //			MyBarRect(1,1,1400, 1000,COLORREF
      // RGB(200,200,200));
    ...
    case WM_LBUTTONDOWN:
      ...
      MyBarRect (X0Options, Y0Options, X0Options + OptWidth,
                 Y0Options + OptHeight, COLORREF RGB (240, 240, 240));
      if ((xc > X0Thinking + ThWidth - 15 * ax)
          && (xc < (X0Thinking + ThWidth)) && (yc > (Y0Thinking))
          && (yc < (Y0Thinking + 15 * ay)))
        {
      ...
      if ((moveMessage[0] > 0) && (gameDepth == treeDepth))
        {
          ...
          //				Sleep(5000);
          //				MessageBox(hWnd, moveMessage, " MAT
          // APROPIAT ", MB_OK);
          // strcpy(moveMessage,"");
        }
...
TestButtons ()
{
  ...
  if ((xc > X0Options) && (xc < (X0Options + OptWidth)) && (yc > (Y0Options))
      && (yc < (Y0Options + OptHeight)))
    {
      // Chess Level
      x = SZSCALE;
      if ((xc > (X0Options + 8 * x)) && (xc < (X0Options + 8 * x + 10 * x))
      ...
      if ((xc > (X0Options + 8 * x)) && (xc < (X0Options + 8 * x + 10 * x))
          && (yc > (Y0Options + 50 * ay)) && (yc < (Y0Options + 75 * ay)))
        {
          ...
          if (Options.agresivity < 0)
            Options.agresivity = 0;
          if (Options.agresivity >= 10)
            Options.agresivity = 9;
          noTime = 1;
        }

      if (indRedraw == 3)
        return 0; // call from GenTree()
...
ExecutePlayButton ()
{
  ...
  aa = MessageBox (hWnd, " Doriti sa jucati o noua partida ? ", " ATENTIE ",
                   MB_YESNO);
...
ExecuteResignButton ()
{
  MessageBox (hWnd, " CEDEZ - Totusi putem sa continuam partida",
              " FELICITARI, ATI CASTIGAT ! ", MB_OK);
      ...
      if (indRedraw < 3)
        {
          ...
          HBITMAP hbm
              = (HBITMAP)LoadImage (hInst, modelList[Options.nModel],
          ...
          //	  MyBarRect(X0Options,Y0Options, X0Options + OptWidth,
          // Y0Options + OptHeight,COLORREF RGB(240,240,255));

          //	  MyBarRect(X0MoveList,Y0MoveList, X0MoveList + MvLstWidth,
          // Y0MoveList + MvLstHeight, RGB(200,240,200));
          //	  MyBarRect(X0Buttons,Y0Buttons, X0Buttons + ButtWidth,
          // Y0Buttons + ButtHeight, RGB(200,200,255));
        }

      sta++;
    }

  MyTextOut (X0Board + 1 * SqSize + SqSize / 2, Y0Board + SqSize - SqSize / 3,
             "A", RGB (0, 0, 0));
  MyTextOut (X0Board + 2 * SqSize + SqSize / 2, Y0Board + SqSize - SqSize / 3,
             "B", RGB (0, 0, 0));
  MyTextOut (X0Board + 3 * SqSize + SqSize / 2, Y0Board + SqSize - SqSize / 3,
             "C", RGB (0, 0, 0));
  MyTextOut (X0Board + 4 * SqSize + SqSize / 2, Y0Board + SqSize - SqSize / 3,
             "D", RGB (0, 0, 0));
  MyTextOut (X0Board + 5 * SqSize + SqSize / 2, Y0Board + SqSize - SqSize / 3,
             "E", RGB (0, 0, 0));
  MyTextOut (X0Board + 6 * SqSize + SqSize / 2, Y0Board + SqSize - SqSize / 3,
             "F", RGB (0, 0, 0));
  MyTextOut (X0Board + 7 * SqSize + SqSize / 2, Y0Board + SqSize - SqSize / 3,
             "G", RGB (0, 0, 0));
  MyTextOut (X0Board + 8 * SqSize + SqSize / 2, Y0Board + SqSize - SqSize / 3,
             "H", RGB (0, 0, 0));

  MyTextOut (X0Board + 1 * SqSize + SqSize / 2,
             Y0Board + 9 * SqSize + SqSize / 6, "A", RGB (0, 0, 0));
  MyTextOut (X0Board + 2 * SqSize + SqSize / 2,
             Y0Board + 9 * SqSize + SqSize / 6, "B", RGB (0, 0, 0));
  MyTextOut (X0Board + 3 * SqSize + SqSize / 2,
             Y0Board + 9 * SqSize + SqSize / 6, "C", RGB (0, 0, 0));
  MyTextOut (X0Board + 4 * SqSize + SqSize / 2,
             Y0Board + 9 * SqSize + SqSize / 6, "D", RGB (0, 0, 0));
  MyTextOut (X0Board + 5 * SqSize + SqSize / 2,
             Y0Board + 9 * SqSize + SqSize / 6, "E", RGB (0, 0, 0));
  MyTextOut (X0Board + 6 * SqSize + SqSize / 2,
             Y0Board + 9 * SqSize + SqSize / 6, "F", RGB (0, 0, 0));
  MyTextOut (X0Board + 7 * SqSize + SqSize / 2,
             Y0Board + 9 * SqSize + SqSize / 6, "G", RGB (0, 0, 0));
  MyTextOut (X0Board + 8 * SqSize + SqSize / 2,
             Y0Board + 9 * SqSize + SqSize / 6, "H", RGB (0, 0, 0));

  MyTextOut (X0Board + 1 * SqSize - SqSize / 3,
             Y0Board + 1 * SqSize + SqSize / 3, "8", RGB (0, 0, 0));
  MyTextOut (X0Board + 9 * SqSize + SqSize / 4,
             Y0Board + 1 * SqSize + SqSize / 3, "8", RGB (0, 0, 0));

  MyTextOut (X0Board + 1 * SqSize - SqSize / 3,
             Y0Board + 2 * SqSize + SqSize / 3, "7", RGB (0, 0, 0));
  MyTextOut (X0Board + 9 * SqSize + SqSize / 4,
             Y0Board + 2 * SqSize + SqSize / 3, "7", RGB (0, 0, 0));

  MyTextOut (X0Board + 1 * SqSize - SqSize / 3,
             Y0Board + 3 * SqSize + SqSize / 3, "6", RGB (0, 0, 0));
  MyTextOut (X0Board + 9 * SqSize + SqSize / 4,
             Y0Board + 3 * SqSize + SqSize / 3, "6", RGB (0, 0, 0));

  MyTextOut (X0Board + 1 * SqSize - SqSize / 3,
             Y0Board + 4 * SqSize + SqSize / 3, "5", RGB (0, 0, 0));
  MyTextOut (X0Board + 9 * SqSize + SqSize / 4,
             Y0Board + 4 * SqSize + SqSize / 3, "5", RGB (0, 0, 0));

  MyTextOut (X0Board + 1 * SqSize - SqSize / 3,
             Y0Board + 5 * SqSize + SqSize / 3, "4", RGB (0, 0, 0));
  MyTextOut (X0Board + 9 * SqSize + SqSize / 4,
             Y0Board + 5 * SqSize + SqSize / 3, "4", RGB (0, 0, 0));

  MyTextOut (X0Board + 1 * SqSize - SqSize / 3,
             Y0Board + 6 * SqSize + SqSize / 3, "3", RGB (0, 0, 0));
  MyTextOut (X0Board + 9 * SqSize + SqSize / 4,
             Y0Board + 6 * SqSize + SqSize / 3, "3", RGB (0, 0, 0));

  MyTextOut (X0Board + 1 * SqSize - SqSize / 3,
             Y0Board + 7 * SqSize + SqSize / 3, "2", RGB (0, 0, 0));
  MyTextOut (X0Board + 9 * SqSize + SqSize / 4,
             Y0Board + 7 * SqSize + SqSize / 3, "2", RGB (0, 0, 0));

  MyTextOut (X0Board + 1 * SqSize - SqSize / 3,
             Y0Board + 8 * SqSize + SqSize / 3, "1", RGB (0, 0, 0));
  MyTextOut (X0Board + 9 * SqSize + SqSize / 4,
             Y0Board + 8 * SqSize + SqSize / 3, "1", RGB (0, 0, 0));
  ...
  if ((indRedraw < 3) || (gameDepth == treeDepth))
    {
      ...
      MyLine (X0Board + SQSIZE, Y0Board + SQSIZE, X0Board + SQSIZE,
              Y0Board + 9 * SQSIZE, RGB (0, 0, 0));
      MyLine (X0Board + SQSIZE, Y0Board + 9 * SQSIZE, X0Board + 9 * SQSIZE,
              Y0Board + 9 * SQSIZE, RGB (0, 0, 0));
      MyLine (X0Board + 9 * SQSIZE, Y0Board + 9 * SQSIZE, X0Board + 9 * SQSIZE,
              Y0Board + SQSIZE, RGB (0, 0, 0));
      MyLine (X0Board + 9 * SQSIZE, Y0Board + SQSIZE, X0Board + SQSIZE,
              Y0Board + SQSIZE, RGB (0, 0, 0));
    }
  if (indRedraw <= 3)
    {
...
DrawChessPiece (int IconSet, int square, int pie)
{
  ...
  if (((i1 + j1) % 2) == 0)
    {
      MyBarRect1 ((i1 + 1) * SqSize + X0Board, (7 - j1 + 1) * SqSize + Y0Board,
                  (i1 + 2) * SqSize + X0Board, (7 - j1 + 2) * SqSize + Y0Board,
                  RGB (170, 170, 170));
    }
  else
    {
  ...
  ind0
      = IconSet * ICONSIZE * ICONSIZE * 3 * 13 + ICONSIZE * ICONSIZE * 3 * pie;
  ind = ind0 + ICONSIZE * 3 * 2
        + 3 * 2; // a point near corner of icon not just on edge

  rgbBk = RGB ((int)IconBits[ind], (int)IconBits[ind + 1],
               (int)IconBits[ind + 2]);
...
RedrawOptions ()
{
  ...
  MyBarRect (X0Options, Y0Options, X0Options + OptWidth, Y0Options + OptHeight,
             COLORREF RGB (240, 240, 240));

  // Redraw Scale of Chess Degrees in Chess

  MyTextOut (X0Options + 10 * ax, Y0Options + 5 * ay,
             " Nivel sahist   :", RGB (0, 0, 0));
  ...
  for (n = 0; n < Options.nLevel; n++)
    {
      ...
      MyCircle (X0Options + 8 * SZSCALE + n * SZSCALE + 10 * ax,
                Y0Options + 17 * ay, 10 * ax, co1);

      sprintf (stri, " %d", n + 1);
      MyTextOut (X0Options + 8 * SZSCALE + n * SZSCALE, Y0Options + 9 * ay,
                 stri, RGB (0, 0, 0));
    }

  MyTextOut (X0Options + 10 * ax, Y0Options + 30 * ay,
             " Nivel de timp :", RGB (0, 0, 0));
  ...
  for (n = 0; n < Options.nLevel; n++)
    {
      ...
      MyCircle (X0Options + 8 * SZSCALE + n * SZSCALE + 10 * ax,
                Y0Options + 42 * ay, 9 * ax, co1);

      sprintf (stri, " %d", n + 1);
      if (n == Options.nLevel - 1)
        strcpy (stri, " T");
      MyTextOut (X0Options + 8 * SZSCALE + n * SZSCALE, Y0Options + 35 * ay,
                 stri, RGB (0, 0, 0));
    }

  MyTextOut (X0Options + 10 * ax, Y0Options + 55 * ay,
             " Agresivitate   :", RGB (0, 0, 0));
  ...
  for (n = 0; n < Options.nLevel; n++)
    {
      ...
      MyCircle (X0Options + 8 * SZSCALE + n * SZSCALE + 10 * ax,
                Y0Options + 67 * ay, 9 * ax, co1);

      sprintf (stri, " %d", n + 1);
      MyTextOut (X0Options + 8 * SZSCALE + n * SZSCALE, Y0Options + 60 * ay,
                 stri, RGB (0, 0, 0));
    }
...
RedrawThinking ()
{
  ...
  if (setup > 0)
    return 1; // no thinking when SETUP

  // if(generatedTotal < 100) return 1;

  MyBarRect (X0Thinking, Y0Thinking, X0Thinking + ThWidth,
             Y0Thinking + ThHeight, RGB (220, 220, 220));
  ...
  sprintf (strline,
           " Nivel : %d  Nr mutari generate : %d  Evaluare : %4.2f  Mutarea "
           "analizata : %d/%d Nivel maxim = %d Ramura Max. = %d   ",
           kmax, nodes, vv, aa, bb, dd, cc);

  MyTextOut (X0Thinking + 10 * ax, Y0Thinking + 10 * ay, strline,
             RGB (0, 0, 128));
  ...
  MyTextOut (X0Thinking + ThWidth - 120 * ax, Y0Thinking + 10 * ay, stime,
             RGB (255, 0, 0));
  ...
  if (findopening == 0 && backmove == 0)
    MyTextOut (X0Thinking + 10 * ax, Y0Thinking + 30 * ay, strline,
               RGB (0, 0, 0));
  ...
  if ((moveMessage[0] > 0) && (gameDepth == td))
    {
      ...
      strcat (strline, movesstring);
      MyTextOut (X0Thinking + 10 * ax + 500 * ax, Y0Thinking + 30 * ay,
                 strline, RGB (255, 0, 0));
    }
  ...
  MyTextOut (X0Thinking + 10 * ax, Y0Thinking + 50 * ay, strline,
             RGB (0, 0, 0));
  ...
  if ((setup == 0) && (setup1 == 0))
      ...
      for (n = 0; n < MoveNumber - 1; n++)
        {
          ...
          ind0 = IconSet * ICONSIZE * ICONSIZE * 3 * 13
                 + ICONSIZE * ICONSIZE * 3 * pie;
          ind = ind0 + ICONSIZE * 3 * 2
                + 3 * 2; // a point near corner of icon not just on edge
                  ...
                  if (pie > 6)
                    {
                      //			 x = (int)(X0Board + dSq * (
                      // ICONSIZE * (i1 + 1) + i)); 			 y =
                      // (int)(Y0Board + dSq * ( ICONSIZE * (8 - j1) + 63 -
                      // j));
  ...
  else
    {
      ...
      MyBarRect (X0Taken + TaWidth / 2.6 * ax, Y0Taken - 10 * ay,
                 X0Taken + TaWidth / 2.6 * ax + 120 * ax, Y0Taken + 10 * ay,
                 RGB (0, 0, 200));
      ...
      MyBarRect (X0Taken + TaWidth / 2.6 * ax + 140 * ax, Y0Taken - 10 * ay,
                 X0Taken + TaWidth / 2.6 * ax + 260 * ax, Y0Taken + 10 * ay,
                 RGB (0, 0, 200));
      ...
      MyBarRect (X0Taken + TaWidth / 2.6 * ax + 300 * ax, Y0Taken - 10 * ay,
                 X0Taken + TaWidth / 2.6 * ax + 420 * ax, Y0Taken + 10 * ay,
                 RGB (0, 0, 200));
      ...
      MyBarRect (X0Taken + TaWidth / 2.6 * ax + 440 * ax, Y0Taken - 10 * ay,
                 X0Taken + TaWidth / 2.6 * ax + 560 * ax, Y0Taken + 10 * ay,
                 RGB (0, 0, 200));
      ...
      MyBarRect (X0Taken + TaWidth / 2.6 * ax + 600 * ax, Y0Taken - 10 * ay,
                 X0Taken + TaWidth / 2.6 * ax + 720 * ax, Y0Taken + 10 * ay,
                 RGB (0, 0, 200));
      ...
      MyTextOut (X0Taken + TaWidth - 700 * ax, Y0Taken + 20 * ay,
                 " <-- Alegeti cu Mouse Tipul Piesei din aceasta caseta, apoi "
                 "click pe Tabla de Sah",
                 RGB (0, 0, 255));
...
RedrawMoveList ()
{
  ...
  if (setup == 0)
    {
      MyBarRect (X0MoveList, Y0MoveList - 16, X0MoveList + 80 + MvLstWidth,
                 Y0MoveList + 20 + MvLstHeight, RGB (200, 220, 200));
      ...
      for (n = numberFrom; n <= (MoveNumber - 2); n = n + 2)
        {
          ...
          MyTextOut (X0MoveList + 10 * ax, Y0MoveList + xrow * 20 * ay,
                     movesstring, RGB (0, 0, 0));
          MyTextOut (X0MoveList + 120 * ax, Y0MoveList + xrow * 20 * ay,
                     blackmove, RGB (0, 0, 0));

          sprintf (movesstring, "%s %s", Stack2[n].info, Stack2[n + 1].info);
          // if(strcpy(movesstring, "() ()") !=0 ) strcpy(movesstring,"");
          MyTextOut (X0MoveList + 30 * ax, Y0MoveList + (xrow + 1) * 20 * ay,
                     movesstring, RGB (0, 0, 255));
          xrow = xrow + 2;
          ;
        }
      indRedraw = 1;
  ...
  else
    {
      ...
      /*
              xrow = 1;
              MyBarRect(X0MoveList,Y0MoveList, X0MoveList + MvLstWidth,
         Y0MoveList + MvLstHeight, RGB(200,220,200)); for(int
         nn=0;nn<=nModels;nn=nn++)
              {
              n1 = nn % 2;
                      hbm = (HBITMAP) LoadImage(hInst, modelList[n],
         IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE); if(hbm)
                      {
                       hBrush = CreatePatternBrush(hbm);
                       DeleteObject(hbm);

                       xa = X0MoveList + 40 * ax + n1 * 50 * ax;
                       ya = Y0MoveList + 10 * ay + 40 * (nn - (nn % 2)) / 2 *
         ay; xb = X0MoveList + 40 * ax + 40 * ax + n1 * 50 * ax; yb =
         Y0MoveList + 10 * ay + 40 * (nn - (nn % 2)) / 2 * ay + 35 * ay;
                      ...
                      MyTextOut(X0MoveList + 5 * ax , Y0MoveList + MvLstHeight
         * ay - 40 * ay, " ALEGETI MODEL DECOR ", RGB(0,0,255));
...
RedrawButtons ()
{
  ...
  MyCircle (X0Buttons + 15 * ax, Y0Buttons + 20 * ay, 8 * ax,
            RGB (100, 100, 250));
  MyTextOut (X0Buttons + 15 * ax + 0.5 * SQSIZE, Y0Buttons + 10 * ay + 2,
             " Partida noua  ", RGB (0, 0, 0));

  MyCircle (X0Buttons + 15 * ax + 0.5 * SQSIZE, Y0Buttons + 50 * ay, 8 * ax,
            RGB (100, 250, 100));
  MyTextOut (X0Buttons + 15 * ax + SQSIZE, Y0Buttons + 40 * ay + 2,
             " Mutare Inapoi     ", RGB (0, 0, 0));

  MyCircle (X0Buttons + 15 * ax + 0.5 * SQSIZE, Y0Buttons + 80 * ay, 8 * ax,
            RGB (100, 250, 100));
  MyTextOut (X0Buttons + 15 * ax + SQSIZE, Y0Buttons + 70 * ay + 2,
             " Mutare Inainte    ", RGB (0, 0, 0));

  MyCircle (X0Buttons + 15 * ax + 0.5 * SQSIZE, Y0Buttons + 110 * ay, 8 * ax,
            RGB (100, 250, 100));
  MyTextOut (X0Buttons + 15 * ax + SQSIZE, Y0Buttons + 100 * ay + 2,
             " Mutare Progr ", RGB (0, 0, 0));

  MyCircle (X0Buttons + 15 * ax, Y0Buttons + 140 * ay, 8 * ax,
            RGB (100, 100, 250));
  MyTextOut (X0Buttons + 15 * ax + 0.5 * SQSIZE, Y0Buttons + 130 * ay + 2,
             " Om vs Om ", RGB (0, 0, 0));

  MyCircle (X0Buttons + 15 * ax, Y0Buttons + 170 * ay, 8 * ax,
            RGB (100, 100, 250));
  MyTextOut (X0Buttons + 15 * ax + 0.5 * SQSIZE, Y0Buttons + 160 * ay + 2,
             " SETARI       ", RGB (0, 0, 0));

  MyCircle (X0Buttons + 15 * ax, Y0Buttons + 200 * ay, 8 * ax,
            RGB (100, 100, 250));
  MyTextOut (X0Buttons + 15 * ax + 0.5 * SQSIZE, Y0Buttons + 190 * ay + 2,
             " Progr vs Progr", RGB (0, 0, 0));

  MyCircle (X0Buttons + 15 * ax, Y0Buttons + 230 * ay, 8 * ax,
            RGB (100, 100, 250));
  MyTextOut (X0Buttons + 15 * ax + 0.5 * SQSIZE, Y0Buttons + 220 * ay + 2,
             " Salvare partida", RGB (0, 0, 0));
...
int
MoveToStringPNG (int Move, char moveout[])
{
  ...
  moveout[2] = (sq1 / 10) + 48 - 1;
  i = 3;
  // moveout[3] = '-'; // intre cele doua patrate este '-' sau 'x' daca se ia
  // ceva
  if (p2 != 0)
    {
      moveout[i] = 'x';
...
int
MyLine (int x1a, int y1a, int x2b, int y2b, COLORREF co1)
{
...
int
MyCircle (int xce, int yce, int raz, COLORREF colo1)
...
int
My2TextOut (int xw11, int yw11, int hiw1, int dwx1, char* buf4a, COLORREF co3a)
{
...
int
image_read (char bu[], int pie, int IconSet)
{
  ...
  nread = fread ((char*)&bmp_input, 1, sizeof (struct bmp_header),
                 f1); // read the header
  ...
  indi0
      = IconSet * ICONSIZE * ICONSIZE * 3 * 13 + pie * ICONSIZE * ICONSIZE * 3;
