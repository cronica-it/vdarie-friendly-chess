...
int
MoveToString (int Move, char[]);
int
MoveToStringPNG (int Move, char[]);
  ...
  for (i = 0; i < 500; i++)
    {
      ...
      Stack[i].mateDetected
          = 0; // if == 1 mate is detected and valable till the end of the game
      ...
      for (int j = 0; j < 13; j++)
        {
          ...
          Stack[i].pieceAPosition[i][j]; // a square where there is a piece of
                                         // that type
        }
      Stack[i]
          .positionVal; // sum of position values of each piece on the board
      Stack[i].PawnHash; // Current Pawn Hash initiated at each search and
                         // updated on PlayMove
      ...
      Stack2[i].nbAttacks
          = 0; // number of attack moves (and rocades, transform etc)
      for (int j = 0; j < 100; j++)
        {
          Stack2[i].Attacks[j] = 0; // all moves al a level
          Stack2[i].ponders[j] = 0; // list of ponders corecponding to Attacks
                                    // list, indexed by rankOnLevel
        }
      Stack2[i].flagsOfGame = 0; // some flags of the game
      Stack2[i].reps = 0; // repetitions;
      Stack2[i].BestLineDepth
          = 0; // the depth of the generated best line vector
      Stack2[i].MaxDepth = 0; // the max value for analized for move in tree
      Stack2[i].EffectiveAnalysed
          = 0; // how many moves where analysed on level 0
...
ExecuteBackButton ()
{
  ...
  if (gameDepth > treeDepth)
    return 1; // MoveBack delayed to Gentree()
...
//
// GamePlaying - loop of two players - one made by man and the computer answer
//
int
GamePlaying (int nply)
{
  ...
  for (tt = 0; tt < nply; tt++)
    {
      ...
      EvaluateAttacks (0, 0, 0, 0, 0,
                       0); // setup attacks tables and estimate exchanges
      ...
      if (WhiteToMove)
        {
          ...
          if ((Stack[gameDepth].pieceCounts[WK] == 0)
              || (Stack[gameDepth].pieceCounts[BK] == 0))
            {
              return 1;
            }
          if (abs (OptimalMoveMoveValue) >= 19997
              || gameDepth > 0 && OptimalMove == 0)
            {
              return -1;
            }

          if ((totalMatePower <= 2) && (Stack[gameDepth].pieceCounts[WB] <= 1)
              && (Stack[gameDepth].pieceCounts[BB] <= 1)
              && abs (Stack[gameDepth].ponder) < 340)
            {
              SaveGame (hWnd);
              strcpy (moveMessage, " REMIZA - NU MAI SUNT PIESE PENTRU MAT");
              MessageBox (hWnd, " REMIZA - NU MAI SUNT PIESE PENTRU MAT ",
                          " REMIZA ", MB_OK);
          ...
          tasta = GetWhiteMove ();
          if (tasta == 0 || gameDepth > 0 && MoveToDo == 0)
            return 0;
          if (tasta > -1)
            {
              MoveNumber++;
          ...
          if (Stack2[gameDepth].flagsOfGame == 44)
            {
              SaveGame (hWnd);
              MessageBox (hWnd, " REPETARE POZITIE DE 3 ORI ",
                          "  SFARSIT PARTIDA ", MB_OK);
              endOfGame = 1;
              return -2;
            }
      ...
      else
        {
          ...
          if (Stack2[gameDepth].flagsOfGame == 66
              || ((totalMatePower <= 2)
                  && (Stack[gameDepth].pieceCounts[WB] <= 1)
                  && (Stack[gameDepth].pieceCounts[BB] <= 1)
                  && abs (Stack[gameDepth].ponder) < 340))
            {
              SaveGame (hWnd);
  ...
  if (Stack[gameDepth].nPieces == 2)
    {
      SaveGame (hWnd);
      strcpy (moveMessage, "");
      MessageBox (hWnd, " REMIZA - NU MAI SUNT PIESE", "SFARSIT PARTIDA",
                  MB_OK);
      endOfGame = 1;
      return -1;
    }
  ...
  if (Stack[gameDepth].pieceCounts[BK] == 0)
    {
      SaveGame (hWnd);
      strcpy (moveMessage, "");
      MessageBox (hWnd, "  EROARE POZITIE - REGE NEGRU LIPSA ",
                  "SFARSIT PARTIDA", MB_OK);
      endOfGame = 1;
      return -1;
    }
  ...
  if (Stack[gameDepth].pieceCounts[WK] == 0)
    {
      SaveGame (hWnd);
      strcpy (moveMessage, "");
      MessageBox (hWnd, "  EROARE POZITIE - REGE ALB LIPSA ",
                  "SFARSIT PARTIDA", MB_OK);
      endOfGame = 1;
      return -1;
    }
  ...
  if (Stack2[gameDepth].flagsOfGame == 44)
    {
      ...
      MessageBox (hWnd, " REMIZA - REPETITIE DE 3 ORI", "SFARSIT PARTIDA",
                  MB_OK);
      endOfGame = 1;
      return -1;
    }
  ...
  if (Stack2[gameDepth].flagsOfGame == 33)
    {
      ...
      MessageBox (hWnd, " REMIZA - REGULA CELOR 50 MUTARI", "SFARSIT PARTIDA",
                  MB_OK);
      endOfGame = 1;
      return -1;
    }
  ...
  if (OptimalMoveMoveValue > (MAT - 100))
    {
      ...
      sprintf (buff, "  Albul da MAT in %d mutari ",
               (MAT - OptimalMoveMoveValue) / 2);
  ...
  if (OptimalMoveMoveValue < (-MAT + 100))
    {
      if (OptimalMoveMoveValue <= (-MAT + 2))
        SaveGame (hWnd);
      sprintf (buff, "  Negrul da MAT in %d mutari ",
               (MAT + OptimalMoveMoveValue) / 2);
      if (OptimalMoveMoveValue >= (-MAT + 100))
        sprintf (buff, "  Posibil MAT Negrul in  %d mutari ",
                 (MAT + OptimalMoveMoveValue - 100) / 2);
  ...
  if ((WhiteIsMan) && (nplyx != 999))
  ...
  else
    {
      ...
      if (rr == 1)
        {
#if 0
          PutMoveString (OptimalMove.SqFrom, OptimalMove.SqTo,
                         OptimalMove.PieceMoved, OptimalMove.PieceTaken,
                         OptimalMove.Indicators);
...
GetMoveString (int& MoveToDo)
{
  ...
  if (((xx == 9) || (xx == 11) || xx == 10) && (PieceMoved == BP)
      && (SqTo < 30))
    {
      Indicators = 9; // black transform in quen
    }
  if (((xx == -9) || (xx == -11) || xx == -10) && (PieceMoved == WP)
      && (SqTo > 90))
    {
...
SaveGame (HWND hwnd)
{
  ...
  // if (GetFileName (hwnd, 1, buffer, saveFile) == TRUE)
  {
    ...
    sprintf (buffer, "Partida la Nivelul %2d Agesivitate %2d  .pgn",
             Options.nTime + 1, Options.agresivity + 1);
    ...
    if (fpgn != NULL)
      {
        ...
        */
        ...
        for (n = numberFrom; n <= (treeDepth - 1); n = n + 2)
          {
            ...
            int no = 0;
            if ((whitemove[0] == '0' && whitemove[1] != '-')
                || (blackmove[0] == '0' && blackmove[1] != '-'))
              no = 1;
            if (no == 0)
              {
        ...
        fclose (fpgn);
        MessageBox (hWnd, " Salvare partida in fisierul .pgn este OK",
                    " Salvare Partida", MB_OK);
        return;
      }
    else
      {
        MessageBox (hWnd, "Eroare salvare partida in fisier",
                    " Salvare Partida", MB_OK);
        return;
      }
  }
...
// ==============================================================
BOOL CALLBACK
PawnDlgProc (HWND hwnd, UINT msg, UINT wparam, LONG lparam)
{
  lparam = lparam;
  int upgrade = WQ;
  switch (msg)
