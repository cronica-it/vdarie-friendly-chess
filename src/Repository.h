...
// Set History 
// 
int  inline SetHistory (const int move,const int &depth, const int &ahead, const char &who, const int &valueCall, int flag)
    {
	if(move == 0 || move == NO_BEST) return 1;
	...
	de = depth;
	if (de > ksup ) de = ksup; // do not use a very long branches
	if(de < 0) de = 1;
	mo = move & 255;
  	
    
	if(SqTo > 100 || SqFrom > 100 || PieceMoved == 0 || PieceMoved > 12 || PieceTaken != 0 && PieceTaken > 12 )
		return 1;
	...
	if(PieceTaken == 0)
	{
		...
		{			
			...
			if( History.whiteHistory [mv] > 4096) History.whiteHistory [mv] -= History.whiteHistory [mv] / 8;
		}
		else 
		{
			History.blackHistory [mv] +=  de * de  + 1;
			if( History.blackHistory [mv] > 4096) History.blackHistory [mv] -= History.blackHistory [mv] / 8;			
		}
	}	
...
*/
	...
	if(( Stack2[gameDepth].PieceTaken != 0) || (Stack2[gameDepth].Indicators != 0)) return 1;
	...
	else if(Stack2[gameDepth].killers[1] == move)
	{
		...
		if(Stack2[gameDepth].killers_count[0] < Stack2[gameDepth].killers_count[1])
		{
			int temp=Stack2[gameDepth].killers_count[0];

			Stack2[gameDepth].killers_count[0]=Stack2[gameDepth].killers_count[1];
